#include "holberton.h"
/* Initializing function backtrack() knowing its a valid function */
int backtrack(char *s1, char *s2, int s1end, int idx2);
/**
 * length - Counts the length of a given string
 * @string: Given string
...
{
	/*Checks if current index of string is not null*/
	if (string[count] != '\0')
...
int check_Wild2(char *s1, char *s2, int idx1, int idx2, int s1Len)
{
	...
	if (s2[idx2] == '*')
	{
		/*If the char after * is another *, increment idx2*/
		if (s2[idx2 + 1] == '*')
			return (check_Wild2(s1, s2, idx1, idx2 + 1, s1Len));
		/*If the char after * is null, it is done iterating and returns 1*/
		if (s2[idx2 + 1] == '\0')
			return (1);
		/* If the char after * does not match s1, start from end of */
...
/**
 * backtrack - The purpose of this function is to backtrack to find a
 * certain letter and provide the indexes when the matching letter is found
 * @s1: string 1
